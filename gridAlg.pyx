...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef float[:,:] removeOtherGalaxies(float[:,:]  mat,float[:,:] segMask,int idGalaxy):
    cdef: 
        int w, h , i, j
        float[:,:] cpyMat
    w, h= len(mat[0]), len(mat)
    ...
    for i in range(w):
        for j in range(h):
            ...
            for dRho in range(len(deltaRadius)):
                nx, ny = ellipse.findPoint(sc, rho+deltaRadius[dRho])
...
cdef float samplingMatrixPoint(float epointx,float epointy, float pointx,float  pointy,float dist,int nsamples):
     ...
     if(getCartesianDist(px+0.5,py+0.5, epointx, epointy)<=dist) and (getCartesianDist(px-0.5,py+0.5, epointx, epointy)<=dist):
         if(getCartesianDist(px+0.5,py-0.5, epointx, epointy)<=dist) and (getCartesianDist(px-0.5,py-0.5, epointx, epointy)<=dist):
                 return 1.0
     if(getCartesianDist(px+0.5,py+0.5, epointx, epointy)>dist) and (getCartesianDist(px-0.5,py+0.5, epointx, epointy)>dist):
         if(getCartesianDist(px+0.5,py-0.5, epointx, epointy)>dist) and (getCartesianDist(px-0.5,py-0.5, epointx, epointy)>dist):
...
cpdef tuple getAccumulatedLuminosity(float[:,:] mat, object ellipse,int nsamples=50, int mindist = 0):
    ...
    w, h = len(mat[0]), len(mat)
    epx, epy = ellipse.posx, ellipse.posy
    ndists = len(mat)/2 -mindist
    ...
    for d in range(ndists):
        ...
        for wit in range(w):
            for hit in range(h):
                ...
                acc[d] = acc[d] + (mat[hit, wit]-isky)*percentage
    return (dists, acc)
